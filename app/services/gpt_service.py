...
class GPTService:
    ...
    async def generate_report(self, chat_logs: List[dict]) -> dict:
        ...
        if not conversation_history.strip():
            print("[GPT_SERVICE_WARN] No conversation history provided to generate report.")
        ...
        # 디버깅을 위해 최종적으로 전달되는 messages 로깅 (선택적)
        # print(f"[GPT_SERVICE_DEBUG] Messages sent to OpenAI: {messages}")
        ...
        try:
            ...
            print(f"[GPT_SERVICE_ERROR] OpenAI API call failed: {e}")
            # API 호출 실패 시 기본 반환 값
            return {
                "situation_summary": "리포트 생성 중 오류가 발생했습니다 (API 호출 실패).",
                "emotion_summary": "", "communication_pattern": "", "recommendations": "",
                "suggest_counseling": False
            }

        print(f"[GPT_SERVICE_DEBUG] Raw GPT content received:\n---\n{gpt_content}\n---")
        return parse_report(gpt_content) # parse_report는 GPT 응답 형식이 위 프롬프트와 일치할 것을 기대